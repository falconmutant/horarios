...
class Aula_Disponibilidad_Detail(APIView):
  def get_object(self,pk):
    try:
      return Aula_Disponibilidad.objects.get(pk=pk)
    except Aula_Disponibilidad.DoesNotExist:
      raise Http404
  ...
  def put(self, request, pk, format=None):
    ...
    serializer = Aula_Disponibilidad_Serializer(aula_disponibilidad, data=request.data)
    if serializer.is_valid():
      serializer.save()
      return Response(serializer.data)
    return Response(serializer.errors, status = status.HTTP_400_BAD_REQUEST)
